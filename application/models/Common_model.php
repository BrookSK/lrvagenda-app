...
class Common_model extends CI_Model {
    ...
    function check_verification_code($code)
    {
        $this->db->select();
        $this->db->from('users');
    ...
    function get_all_company_brands($business_id)
    {
        $this->db->select();
        $this->db->from('brands');
        $this->db->where('business_id', $business_id); 
    ...
    function get_user_testimonials($business_id)
    {
        $this->db->select();
    ...
    function get_company_sliders($business_id,$limit)
    {
        $this->db->select();
    ...
    function get_custom_inputs($business_id,$service_id){
        $this->db->select();
        $this->db->from('custom_form');
        $this->db->where('business_id', $business_id);
        $this->db->where('service_id', 0);
        $this->db->or_where('service_id', $service_id);
        $this->db->where('status', 1);
        $query = $this->db->get();
        $query = $query->result();  
        return $query;
    ...
    function get_extra_services($id)
    {
        $this->db->select('*');
        $this->db->from('service_extra');
    ...
    function get_company_services($business_id, $limit)
    {
        ...
        if (!empty($_GET['categories'])) {
            //print_r(json_encode($_GET['staffs']));
            $this->db->like('category_id', json_decode($_GET['categories']));
        }
    ...
    // select function
    function get_user_single_page($slug, $buid)
    {
        $this->db->select();
        $this->db->from('pages');
        $this->db->where('slug', $slug);
    ...
    //get_by_slug($this->session->userdata('site_lang'), 'language')->id
    ...
    function get_site_blog_posts($total, $limit, $offset, $business_id){
...
public function Is_already_register($email){
   ...
   if($query->num_rows() > 0){
        ...
        if(empty($row->device_1)){
            $updater['device_1'] = $deviceToken;
        }else{
            $updater['device_2'] = $deviceToken;
        }
...
public function Update_user_data($data, $id){
  ...
  $query = $this->db->get_where('users', array('auth_id' => $id));
...
// {
//         $this->db->select('r.*');
//         $this->db->select_sum('r.rating', 'total');
//         $this->db->from('ratings r');
//         $this->db->where("r.business_id", $business_id);
//         $query = $this->db->get();
//         $query = $query->result();
...
//  function get_count_rating($business_id)
// {
//         $this->db->select('count(business_id) as total,');
